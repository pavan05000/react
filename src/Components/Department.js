...
const Department = () => {
  ...
  const handleEdit = (id) => {  console.log(id);
    ...
    const formElement = document.getElementById('departmentForm');
    if (formElement) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`http://192.168.1.163:8092/department/${id}`); // Replace with your actual API endpoint
  ...
  const getFilteredList = async (searchData) => {
    ...
    const result = users.filter((data) => {
      return (
        ...
        (data.departmentTitle.toString().includes(searchData))
      );
    });
                      ...
                      <div className='row'>
                        <div className='col-12 col-md-6 col-lg-4 mb-2'>
                          <input
                            ...
                            id='departmentTitle'
                            {...register("departmentTitle", { required: true })}
                          />
                          {errors.departmentTitle && (
                            <p className='errorMsg'>Enter Department</p>
                          )}
                        </div>
                ...
                <div className="card flex-fill">
                  ...
                  {/* <table className="table table-hover my-0">
                    <thead>
                      <tr>
                        <th >S No</th>
                        <th className="d-none d-xl-table-cell">Department</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
